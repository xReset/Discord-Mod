...
/** Refuse to patch while Discord.exe is live (Windows asar lock / corrupt risk). */
function assertDiscordQuit() {
  try {
    ...
    if (/Discord\.exe/i.test(out)) {
      fail("Discord is running — fully quit (tray icon → Quit), then re-run node install.js");
    }
  } catch (e) {
...
function discordAppInfo() {
  ...
  const version = path.basename(newest).replace(/^app-/, "");
  return { resources: path.join(newest, "resources"), version };
}

function buildShim(tmpDir, origAsarPath, originalMain, patchedBuild) {
  ...
  const BUILD = JSON.stringify(patchedBuild || "unknown");
...
const RENDERER_PATH = ${RENDERER};
const DCMOD_PATCHED_BUILD = ${BUILD};
...
async function main() {
  ...
  buildShim(tmpDir, backup, originalMain, version);

  await asar.createPackage(tmpDir, appAsar);
  fs.rmSync(tmpDir, { recursive: true, force: true });

  console.log("\n[install] DONE ✓  (patched build " + version + ")");
...
module.exports = { buildShim, assertDiscordQuit, discordAppInfo };
